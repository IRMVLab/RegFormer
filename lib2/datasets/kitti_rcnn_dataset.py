...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_sample(self, index):
        ...
        pts_rect = pts_rect[pts_valid_flag][:, 0:3]
        pts_intensity = pts_intensity[pts_valid_flag]
        pts_in_img = pts_img[pts_valid_flag]#将点云投影到图像中，得到correspondence

        # 在这里加上RGB信息。
        shape = self.image_hw_with_padding_np
        pts_rgb = interpolate_img_by_xy(img, pts_in_img, shape) #利用插值方式，得到每一个点的语义特征
        ...
        if self.mode == 'TRAIN' or self.random_select:
            if self.npoints < len(pts_rect):
                pts_depth = pts_rect[:, 2]
                pts_near_flag = pts_depth < 40.0 #40m分类为前景与后景
                far_idxs_choice = np.where(pts_near_flag == 0)[0]
                near_idxs = np.where(pts_near_flag == 1)[0]
                near_idxs_choice = np.random.choice(near_idxs, self.npoints - len(far_idxs_choice), replace = False)
