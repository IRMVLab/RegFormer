...
def ProjectPCimg2SphericalRing(PC, Feature = None, H_input = 32, W_input = 1800):
...
def PreProcess(PC_f1, PC_f2, T_gt, T_trans, T_trans_inv, aug_frame):    ####    pre process procedure
    ...
    for i in range(batch_size):
        ...
        # cur_PC_f1_concat = torch.where(r_f1 < 10, torch.zeros_like(cur_PC_f1_concat).cuda(), cur_PC_f1_concat).to(
        #     torch.float32)
        # cur_PC_f1_concat = torch.where(x1 > 40, torch.zeros_like(cur_PC_f1_concat).cuda(), cur_PC_f1_concat).to(torch.float32)
        PC_mask_valid1 = torch.any(cur_PC_f1_concat != 0, dim=-1).cuda().detach()  # H W
        cur_PC_f1_concat = cur_PC_f1_concat[PC_mask_valid1 > 0,:]

        x2 = cur_PC_f2_concat[:, :1]
        ...
        # cur_PC_f2_concat = torch.where(r_f2 < 10, torch.zeros_like(cur_PC_f2_concat).cuda(), cur_PC_f2_concat).to(
        #     torch.float32)
        # cur_PC_f2_concat = torch.where(x2 < -40, torch.zeros_like(cur_PC_f2_concat).cuda(), cur_PC_f2_concat).to(torch.float32)
