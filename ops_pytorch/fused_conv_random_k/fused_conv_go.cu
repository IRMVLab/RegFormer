...
void FusedConvRandomKLauncher(int batch_size, int H, int W, int npoints, int kernel_size_H, 
	...
	fused_conv_random_k_gpu<<<batch_size,512,0,stream>>>(batch_size, H, W, npoints, kernel_size_H, kernel_size_W, K, flag_copy, distance, stride_h, stride_w, xyz1, xyz2, idx_n2, random_hw, selected_b_idx, selected_h_idx, selected_w_idx, valid_idx, valid_in_dis_idx, selected_mask, small_h, small_w);
