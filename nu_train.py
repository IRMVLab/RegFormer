...
from configs import translo_args
...
from nuscenes_pytorch import points_dataset
...
if not args.task_name:
    file_dir = os.path.join(experiment_dir, '{}_Nuscenes_{}'.format(args.model_name, str(
...
os.system('cp %s %s' % ('nu_train.py', log_dir))
os.system('cp %s %s' % ('configs.py', log_dir))
os.system('cp %s %s' % ('translo_model.py', log_dir))
os.system('cp %s %s' % ('translo_model_utils.py', log_dir))
os.system('cp %s %s' % ('conv_util.py', log_dir))
os.system('cp %s %s' % ('nuscenes_pytorch.py', log_dir))
...
def main():
    ...
    for epoch in range(init_epoch + 1, args.max_epoch):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            pos2, pos1, T_gt, T_trans, T_trans_inv = data
...
def eval_pose(model, test_list, epoch):
        ...
        print('Registration Recall {}:' + str(success_rate * 100) + '%')
        print('trans_mean {}:' + str(trans_mean))
        print('trans_std {}:' + str(trans_std))
        print('rot_mean {}:' + str(rot_mean))
        print('rot_std {}:' + str(rot_std))

        avg_time = total_time / 5712
        # print(avg_time)

        # fname_txt = os.path.join(log_dir, str(item).zfill(2) + '_pred.npy')
        data_dir = os.path.join(eval_dir, 'translonet_nuscenes')
